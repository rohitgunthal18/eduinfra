...
export default function AdminDashboard() {
  ...
  const fetchQuotes = async () => {
    try {
      const data = await apiClient.getQuotes(filter === 'all' ? null : filter);
      if (data.success) {
  ...
  const fetchReviews = async () => {
    try {
      const data = await apiClient.getReviews(reviewFilter === 'all' ? null : reviewFilter);
      if (data.reviews) {
  ...
  const updateReviewStatus = async (id, status) => {
    try {
      await apiClient.updateReviewStatus(id, status);
      await fetchReviews();
